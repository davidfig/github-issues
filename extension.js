...
async function open()
{
    ...
    if (repository.url)
    {
        ...
        else if (repository.url.indexOf('git+ssh://git@gitub.com/') === 0)
        {
            const remamining = repository.url.substring(('git+ssh://git@github.com/').length)
            const names = remaining.split('/')
            let uri = Uri.parse('https://github.com/' + names[0] + '/' + names[1].replace('.git', '') + '/issues/')
            commands.executeCommand('vscode.open', uri)
        }
        ...
        {
            window.showErrorMessage('package.json repository.url entry should be in form of "https://github.com/user/repository.git" or "git+ssh://git@github.com/davidfig/respository.git"')
        }
    }
    else if (repository)
    {
